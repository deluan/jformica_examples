...
public class MultiHeartRateMonitor {
    private static final String HRM_NETWORK_NAME = "N:ANT+";
    private static final int[] HRM_PUBLIC_KEY = new int[]{0xB9, 0xA5, 0x21, 0xFB, 0xBD, 0x72, 0xC3, 0x45};
	...
	public static void setupHrmChannel(Channel channel, String name, ChannelId id) {
		...
		channel.assign(HRM_NETWORK_NAME, channelType);
		
		
		// registers an instance of our callback with the channel
		channel.registerRxListener(new ChannelListener(name), BroadcastDataMessage.class);
        channel.registerEventHandler(new DataChannelEventHandler(channel));
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// ANT+ key 
		NetworkKey key = new NetworkKey(HRM_NETWORK_NAME, HRM_PUBLIC_KEY);
